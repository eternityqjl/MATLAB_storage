...
function launcher_OpeningFcn(hObject, eventdata, handles, varargin)
    ...
    a=imread('frameWork.jpg');                  % 寰楀埌a鍥??1?7
    axes(handles.axes1);                        % 鍙栧潗鏍囪酱1
    imshow(a);                                  % 鏄剧ずa鍥??1?7
    global flag_huitu                           % 鏄惁鍋氬瓙鍥炬爣蹇??
    flag_huitu=0;                               % 鍒濆??间负0
...
function y_zssy=zsxyfx(A,B,C,D)
        ...
        pop3Value = get(handles.popupmenu3,'value');% 获取算法弹出框中的选项,噪声强度
        pop4Value = get(handles.popupmenu4,'value');% 获取算法弹出框中的选项,噪声类型
        global y_jb;                                % 定义全局变量, 时间数据
        global t_jb;                                % 阶跃响应
        sys=ss(A,B,C,D);                           % 得到系统
...
function ztxs(t,y,line,hold_flag)
        subplot(321);                               % 鍋氬瓙鍥??
        if(hold_flag==1)                            % 鑻old鏍囧織涓??            hold on                                 % 淇濇寔绐楁牸
        else                                        % hold鏍囧織涓嶄负1
            hold off                                % 涓嶄繚鎸佺獥鏍??1?7
        end
        plot(t, y(:,1),line)                        % 鍋氬嚭浣嶇Щ鐨勫浘鍍??1?7
        title('Displacement')                       % 鍔犲叆鏍囬
        grid on                                     % 鍔犲叆缃戞牸绾??1?7
        set(gca, 'linewidth', 0.9, 'fontsize', 9, 'fontname', ...
            'times','fontangle','italic','XMinorTick'...
            ,'on','YMinorTick','on')                % 璁剧疆鍥惧儚绐楁牄1?7
        subplot(322);                               % 鍋氬瓙鍥??
        if(hold_flag==1)                            % 鑻old鏍囧織涓??            hold on                                 % 淇濇寔绐楁牸
        else                                        % hold鏍囧織涓嶄负1
            hold off                                % 涓嶄繚鎸佺獥鏍??1?7
        end
        plot(t, y(:,2),line)                        % 鍋氬嚭鍥惧儚
        title('speed')                              % 娣诲姞閫熷害鏍囬1?7
        grid on                                     % 鍔犲叆缃戞牸绾??1?7
        set(gca, 'linewidth', 0.9, 'fontsize', 9, 'fontname', ...
            'times','fontangle','italic','XMinorTick'...
            ,'on','YMinorTick','on')                % 璁剧疆鍥惧儚绐楁牄1?7
        ...
        plot(t, y(:,3),line)                        % 鍋氱怄1?7瀛愯搴﹄1?7浘鍍??
        title('rope angle')                         % 娣诲姞鏍囬
        grid on                                     % 鍔犲叆缃戞牸绾??1?7
        set(gca, 'linewidth', 0.9, 'fontsize', 9, 'fontname', ...
            'times','fontangle','italic','XMinorTick'...
            ,'on','YMinorTick','on')                % 璁剧疆鍥惧儚绐楁牄1?7
        ...
        plot(t, y(:,4),line)                        % 鍋氬浘鍍??
        title('rope angle speed')                   % 娣诲姞鏍囬
        grid on                                     % 鍔犲叆缃戞牸绾??1?7
        set(gca, 'linewidth', 0.9, 'fontsize', 9, 'fontname', ...
            'times','fontangle','italic','XMinorTick'...
            ,'on','YMinorTick','on')                % 璁剧疆鍥惧儚绐楁牄1?7
        ...
        plot(t, y(:,5),line)                        % 鍋氬浘鍍??
        title('driving force')                      % 娣诲姞鏍囬
        grid on                                     % 鍔犲叆缃戞牸绾??1?7
        set(gca, 'linewidth', 0.9, 'fontsize', 9, 'fontname', ...
            'times','fontangle','italic','XMinorTick'...
            ,'on','YMinorTick','on')                % 璁剧疆鍥惧儚绐楁牄1?7
...
function fuzzyPID_Callback(hObject, eventdata, handles)
    global time;
    time=0;                                             % 瀹氫箟璁＄畻娆℃暟锛屽垵鍊间贄1?7    KK_yc=[3.2966 18.1250 -18.9404 -117.5919 0.0001];   % 寰楀埌鍒濆????1?7
    pop11Value = get(handles.popupmenu1,'value');      % 鑾峰彇绠楁硶寮瑰嚭妗嗕腑鐨勯??夐?1?7?1?7
    %% 鍒ゆ柇涓嬫媺鑿滃崟鐨勯??夋嫨锛屽苟寰楀埌KK鍊??1?7
    switch pop11Value                                   % 瀵?1?7?笅鎷夎彍鍗曡繘琛屽垽鏂??
        case 1                                          % 榛樿浣跨敤鏈??浼樺????1?7
            KK=[3.2966 18.1250 -18.9404 -117.5919 0.0001];
        case 2                                          % 鐩存帴浣跨敤鏈??浼楰K鍊??1?7
            KK=[3.4966 16.1250 -18.9404 -117.5919 0.0001];
        case 3                                          % 浣跨敤ga鍑芥暟璁＄畻KK鍊??1?7
...
%  函数功能：实现对于系统的基本参数的设定以及判断系统的能控能观和稳定性????
function pushbutton1_Callback(hObject, eventdata, handles)
    ...
    [A, B, C, D] = tf2ss(num, den)                  % 传递函数转状态空间
